...
Having printed the model summary we can see that the training found K = 7 as the optimum setting.

```{r}
predicted_classes <- predict(knn_model, newdata = as.matrix(XTest[2:30]))

(results <- confusionMatrix(predicted_classes, as.matrix(XTest[31])  ))
```

As you can see from the summary the model accuracy with the test set is approximately 99.5% which is quite impressive.
