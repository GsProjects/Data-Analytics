...
{
  for (column in names(temporary_data[1:30]) )
    {
...
start_time <- Sys.time()
model <- knn(as.matrix(XTrain[1:30]),test = as.matrix(XTest[1:30]),
...
The CrossTable function gives a description of the predictions by does not actually give an accuracy score for the model. This has to be calculated manually unfortunately. In this case the accuracy score of the model is approximately 99.61%. The accuracy is calculated by adding the total number of correct predictions for valid transactions with the total number of correct predictions for fraudulent transactions and then dividing that by the total number of observations.
