...
cat('The total time to train the model and predict test classes was: ', total_execution)
```
...
training_control <- trainControl(method ='repeatedcv', repeats = 2)
...
cat('The total time to train the model was: ', total_execution)
```

Using the caret package for KNN is considerably faster that R's base KNN class. This may be due to 10 fold cross-validation which splits the training sample into 10 samples. One of these samples is then used as a test set. Therefore 90% of the data is used for training. This process is repeated twice.

```{r}
knn_model
```

Having printed the model summary we can see that the training found K = 9 as the optimum setting.
...
logistic_predicted <- predict(logistic_model, type = 'response', newdata = XTest[2:30] ) # type = response means the output is returned as P(Y =1 | X)
