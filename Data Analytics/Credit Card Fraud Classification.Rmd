...
start_time <- Sys.time()
normalised <- normalise_data(sample_data)
end_time <- Sys.time()
total_execution <- end_time - start_time
cat('The total time to normalise the data was: ', total_execution)
normalised
```
...
set.seed(2)
train_index <- createDataPartition(normalised$Class, times=1, p=.75, list = FALSE)
XTrain <- normalised[ train_index,]

XTest  <- normalised[-train_index,]
```
...
basic_knn <- function(XTrain, XTest)
{
  ...
  set.seed(4)
  model <- knn(as.matrix(XTrain[2:30]),test = as.matrix(XTest[2:30]),
               cl = as.matrix(XTrain[31]) , k=5, prob = TRUE)
...
model <- basic_knn(XTrain, XTest)

end_time <- Sys.time()

total_execution <- end_time - start_time

CrossTable(as.matrix(XTest[31]), y = model )
  
cat('The total time to train the basic knn model and predict test classes was: ', total_execution)
...
(knn_model <- knn_cv(XTrain, XTest))

end_time <- Sys.time()

total_execution <- end_time - start_time
  
cat('The total time to train the cross validated knn model and predict test classes was: ', total_execution,'\n')
```
...
(results <- confusionMatrix(predicted_classes, as.matrix(XTest[31])  ))
```
...
logistic <- function(XTrain, XTest)
{
  start_time <- Sys.time()
...
(logistic_model <- logistic(XTrain, XTest))

logistic_predicted <- predict(logistic_model,type = 'response', newdata = XTest[2:30] ) # if you use the parameter type = response, it means the output is returned as P(Y =1 | X) so you need to convert to probabilites to 0 or 1 using a threshold like below
...
(confusionMatrix(logistic_predicted, as.matrix(XTest[31]) ))
...
```{r}
logistic_cv <- function(XTrain, XTest)
{
  library(gmodels)
  library(class)
...
(log_model <- logistic_cv(XTrain,XTest) )

```

Using repeated cross validation with 5 repeats and 10 folds per repeat the overall model accuracy is 99.621%. This model now needs to be validated against XTest.
...
(confusionMatrix(as.matrix(logistic_predicted), as.matrix(XTest[31]) ))
```
